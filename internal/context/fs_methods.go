...
// FSWrapper provides safe file system operations
...
// NewFSWrapper creates a new file system wrapper
func NewFSWrapper(staticDir, dataDir string, logger Logger) *FSWrapper {
	return &FSWrapper{
		staticDir: staticDir,
		dataDir:   dataDir,
		logger:    logger,
...
func (fw *FSWrapper) ReadStaticFile(relativePath string) ([]byte, error) {
	return fw.readFile(fw.staticDir, relativePath, "static")
}
...
func (fw *FSWrapper) ReadDataFile(relativePath string) ([]byte, error) {
	return fw.readFile(fw.dataDir, relativePath, "data")
}

// readFile is the internal helper that performs the actual read with safety checks
func (fw *FSWrapper) readFile(baseDir, relativePath, dirType string) ([]byte, error) {
	if baseDir == "" {
	...
	if filepath.IsAbs(cleanPath) || strings.HasPrefix(cleanPath, "..") || strings.Contains(cleanPath, "../") {
		fw.logger.Warn("Attempted path traversal blocked", "dir", dirType, "requested", relativePath, "cleaned", cleanPath)
	...
	if !strings.HasPrefix(resolvedPath, baseAbs+string(os.PathSeparator)) && resolvedPath != baseAbs {
		fw.logger.Warn("Path traversal attempt detected after resolution", "base", baseAbs, "resolved", resolvedPath)
	...
	fw.logger.Debug("File read successfully", "dir", dirType, "path", relativePath, "size", len(data))
	return data, nil
}
...
func (fw *FSWrapper) ReadStaticFileStream(relativePath string) (io.ReadCloser, error) {
	return fw.openStream(fw.staticDir, relativePath, "static")
}

// ReadDataFileStream opens a stream to read a file from the data directory.
func (fw *FSWrapper) ReadDataFileStream(relativePath string) (io.ReadCloser, error) {
	return fw.openStream(fw.dataDir, relativePath, "data")
}

// openStream is the internal helper for streaming file access
func (fw *FSWrapper) openStream(baseDir, relativePath, dirType string) (io.ReadCloser, error) {
	if baseDir == "" {
		return nil, fmt.Errorf("%s directory not configured", dirType)
	}
	...
	if filepath.IsAbs(cleanPath) || strings.HasPrefix(cleanPath, "..") || strings.Contains(cleanPath, "../") {
		fw.logger.Warn("Attempted path traversal blocked (stream)", "dir", dirType, "requested", relativePath)
		return nil, fmt.Errorf("invalid path: path traversal detected")
	}
	...
	if !strings.HasPrefix(resolvedPath, baseAbs+string(os.PathSeparator)) && resolvedPath != baseAbs {
		fw.logger.Warn("Path traversal attempt detected (stream)", "base", baseAbs, "resolved", resolvedPath)
		return nil, fmt.Errorf("access denied: path outside allowed directory")
	}
	...
	fw.logger.Debug("File stream opened", "dir", dirType, "path", relativePath)
	return file, nil
}

// GetStaticDir returns the absolute path to the static directory
func (fw *FSWrapper) GetStaticDir() string {
	return fw.staticDir
}

// GetDataDir returns the absolute path to the data directory
func (fw *FSWrapper) GetDataDir() string {
	return fw.dataDir
}

// FileExists checks if a file exists in the static or data directory
func (fw *FSWrapper) FileExists(relativePath string, dirType string) (bool, error) {
	var baseDir string
	if dirType == "static" {
		baseDir = fw.staticDir
